...
class MockCurrentWeatherRepository: CurrentWeatherRepository {
  ...
  func currentWeather(in city: String, completion: @escaping (CurrentWeather) -> Void) {
    callCount += 1
    if callCount > 4 {
      completion(CurrentWeather(weather: [Weather(id: 1, main: "hi", description: "hi", icon: "hi")], main: MainWeatherInfo(temp: Double.random(in: 0.0...7.7), feelsLikeTemp: 1.2, tempMin: 1.3, tempMax: 1.4, pressure: 1, humidity: 7.5, seaLevel: nil, groundLevel: nil), wind: Wind(speed: 1.2, degree: 20, gust: 1.9), id: 5, name: "hihihi"))
    } else {
      completion(weather)
    }
